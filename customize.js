...
// kutsutaan kun customize avataan
document.getElementById("customBtn").onclick = () => {
    mainMenu.style.display="none";
    customOverlay.style.display="flex";
    drawPreview();
...
vaultBtn.onclick = () => {
    ...
    vaultOverlay.style.background = "gray";
    vaultInput.value = "";
    vaultInput.focus();
};
...
vaultInput.addEventListener("keydown", (e)=>{
    if(e.key === "Enter"){
        if(vaultInput.value === "justfordevs"){
            vaultOverlay.style.background = "green";
        }else{
            vaultOverlay.style.background = "red";
        }
    }
});

// sulku klikkaamalla taustaa
